...
/**
 * 与 {@code src/main/java} 下实现保持一致（镜像副本）；兼容旧键 {@code login_tokens:} / {@code login_user_active:}。
 */
@Service
public class LoginSessionRedisService {
    ...
    public void saveOrReplaceSession(Long userId, String token, String loginJsonPayload, Duration ttl) {
        if (userId != null) {
            ...
            String oldToken = firstNonBlank(redisOperatorService.get(LoginRedisKeys.userActiveKey(uid)), redisOperatorService.get(LoginRedisKeys.legacyUserActiveKey(uid)));
    ...
    public void invalidateByUserId(String userIdStr) {
        ...
        String jwt = firstNonBlank(redisOperatorService.get(LoginRedisKeys.userActiveKey(id)), redisOperatorService.get(LoginRedisKeys.legacyUserActiveKey(id)));
        ...
        redisOperatorService.delete(LoginRedisKeys.userActiveKey(id));
