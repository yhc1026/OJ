...
| Data ID | 格式 | 谁在用 | 说明 |
|---------|------|--------|------|
| `common-datasource.yaml` | YAML | `oj-system`（shared-config） | MySQL + **HikariCP** 数据源 |
| `common-mybatis.yaml` | YAML | `oj-system`（shared-config） | **MyBatis-Plus** 公共项 |
| `common-redis.yaml` | YAML | `oj-system`（shared-config） | **Redis**（登录态等） |
...
| `common-redis.yaml` | YAML | `oj-system`（shared-config）**与 `oj-gateway`（第二项 import）** | 同一份 Redis，避免网关密码为空导致 NOAUTH |
